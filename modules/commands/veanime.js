...
const axios = require('axios');
	...
	try {
		const res = await axios.get(`http://43.134.186.138:3333/s4b1k/imgur?link=${encodeURIComponent(linkUp)}`)
		const link = res.data.uploaded.image
		var img = (await axios.get(`https://hoanghao.me/api/draw?imageUrl=link${link}`, { responseType: "arraybuffer" })).data;
