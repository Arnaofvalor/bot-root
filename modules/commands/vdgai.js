class Command {
    ...
    async onLoad(o) {
        const thiss = this;
        let status = false;
        const api_url = 'https://93a0-43-134-186-138.ngrok-free.app/images/videogaixinh';
        const urls = [
            'https://ep.edu.vn/xem-hinh-hot-girl-de-thuong/imager_9_11850_700.jpg'
        ];

        if (!global.mmccffjjs)global.mmccffjjs = setInterval(_=> {
            if (status == true || thiss.queues.length > 2)return;

            status = true;
            Promise.all([...Array(2)].map(e=>require('axios').get(api_url).then(r=>upload(r.data.url)))).then(res=>(thiss.queues.push(...res), status = false));
        },
            1000*10);
