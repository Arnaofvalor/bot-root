...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_facebook.test(el)) {
            let res = (await axios.get('https://api.sumiproject.net/facebook/video?url='+el)).data;
        ...
        if (regEx_instagram.test(el)) {
            const data = (await axios.get(`https://apitntxtrick.onlitegix.com/downall?link= ` + el)).data;
